...
const { jsonFileToObject, getPrettyJson } = require("../utils");
...
class ConfigOperator {
  constructor() {
    ...
    // 配置文件名称
    this.filename = path.resolve(this.cacheDir, CONFIG_FILE_NAME);
    // 默认配置
    this.defaultData = {
  ...
  check() {
    ...
    // 获取本地配置
    this.localData = jsonFileToObject(this.filename);
    // 如果是空则写入默认配置
    if (!this.localData) {
  ...
  // 写入配置文件
  writeFile(data) {
    fs.writeFileSync(this.filename, getPrettyJson(data));
  }
